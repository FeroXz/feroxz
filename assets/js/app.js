import defaultStateSeed from './default-state.js';
...
const defaultState = (() => {
  const seeded = deepClone(defaultStateSeed);
  if (typeof window !== 'undefined' && window.__FEROXZ_INITIAL_STATE__) {
    return mergeDeep(seeded, window.__FEROXZ_INITIAL_STATE__);
  }
  return seeded;
})();

const syncStateWithServer = (payload, { reset = false } = {}) => {
  if (typeof fetch !== 'function') {
    return;
  }
  const endpoint = reset ? '/api/state/reset' : '/api/state';
  const bodyPayload = deepClone(payload ?? {});
  if (reset) {
    fetch(endpoint, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(bodyPayload)
    }).catch((error) => console.warn('Synchronisation mit dem Server fehlgeschlagen', error));
    return;
  }
  if (!reset && typeof navigator !== 'undefined' && typeof navigator.sendBeacon === 'function') {
    try {
      const blob = new Blob([JSON.stringify(bodyPayload)], { type: 'application/json' });
      if (navigator.sendBeacon(endpoint, blob)) {
        return;
      }
    } catch (error) {
      console.warn('Synchronisation via sendBeacon fehlgeschlagen', error);
    }
  }
  fetch(endpoint, {
    method: 'POST',
...
const resetState = () => {
  ...
  syncStateWithServer(defaultState, { reset: true });
  renderAll();
  updateParentChips('one');
  updateParentChips('two');
  calculateGenetics();
