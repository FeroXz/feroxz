...
<div class="grid" style="grid-template-columns:2fr 1fr;gap:2rem;align-items:start;">
            ...
            <tbody>
                <?php foreach ($animals as $animal): ?>
                    <tr>
                        ...
                        <td>
                            <a class="btn btn-secondary" href="<?= route_url('admin/animals', ['edit' => $animal['id']]) ?>">Bearbeiten</a>
                            <a class="btn btn-secondary" href="<?= route_url('admin/animals', ['delete' => $animal['id']]) ?>" onclick="return confirm('Tier wirklich löschen?');">Löschen</a>
                        </td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
    ...
    <div class="card">
        ...
        <form method="post" enctype="multipart/form-data">
            ...
            <label>Genetikprofil (für Rechner)
                <select name="genetics_profile[]" multiple size="10">
                    <?php foreach ($species as $spec): ?>
                        <optgroup label="<?= htmlspecialchars($spec['name']) ?>">
                            <?php foreach ($geneOptions[$spec['slug']] as $options): ?>
                                <?php foreach ($options as $option): ?>
                                    <option value="<?= htmlspecialchars($option['value']) ?>" <?= in_array($option['value'], (array)($editAnimal['genetics_profile'] ?? []), true) ? 'selected' : '' ?>><?= htmlspecialchars($option['label']) ?></option>
                                <?php endforeach; ?>
                            <?php endforeach; ?>
                        </optgroup>
                    <?php endforeach; ?>
                </select>
                <small class="text-muted">Mehrfachauswahl: Strg/Cmd halten. Nur ausgewählte Werte werden im Rechner angeboten.</small>
            </label>
            <label>Herkunft
