...
if (!requirementsAreMet($requirements)) {
    ...
    echo "Systemvoraussetzungen nicht erfüllt. Bitte prüfe folgende Punkte:\n\n";
    if (isset($requirements['extensions'])) {
        foreach ($requirements['extensions'] as $extension) {
            $status = !empty($extension['met']) ? 'OK' : 'FEHLT';
            $label = $extension['label'] ?? 'Erweiterung';
            echo "- {$label}: {$status}\n";
        }
        echo "\n";
    }
    if (isset($requirements['paths'])) {
        foreach ($requirements['paths'] as $path) {
            $status = !empty($path['met']) ? 'OK' : 'NICHT BESCHREIBBAR';
            $label = $path['label'] ?? ($path['path'] ?? 'Pfad');
            $location = $path['path'] ?? '';
            echo "- {$label} ({$location}): {$status}\n";
        }
    }
    exit;
}
...
function ensureDefaultAdmin(PDO $pdo): void
{
    ...
    createAdmin($pdo, $defaultUsername, '12345678');
}
