...
switch ($route) {
    ...
    case 'animals':
        ...
        break;
    case 'adoption':
        render('adoption/index', [
            'adoptions' => fetchPublicAdoptions($pdo)
        ]);
        break;
    case 'adoption/inquire':
        handleAdoptionInquiry($pdo, $method);
        break;
    case 'genetics':
    ...
    case 'admin':
        ...
        renderAdmin('dashboard', [
            ...
            'adoptionCount' => countAll($pdo, 'adoption_animals'),
            'inquiryCount' => countAll($pdo, 'adoption_inquiries')
        ]);
        break;
    case 'admin/posts':
    ...
    case 'admin/animals':
        ...
        break;
    case 'admin/adoptions':
        requireAdmin($pdo, 'adoptions');
        handleAdoptions($pdo, $method);
        break;
    case 'admin/settings':
        requireAdmin($pdo, 'settings');
...
function initializeDatabase(PDO $pdo): void
{
    ...
    $pdo->exec('CREATE TABLE IF NOT EXISTS adoption_animals (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        name TEXT NOT NULL,
    ...
    $pdo->exec('CREATE TABLE IF NOT EXISTS adoption_inquiries (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        adoption_id INTEGER,
        adoption_name TEXT NOT NULL,
        interested_name TEXT NOT NULL,
        interested_email TEXT NOT NULL,
        message TEXT,
        created_at TEXT NOT NULL,
        status TEXT NOT NULL DEFAULT "new",
        FOREIGN KEY (adoption_id) REFERENCES adoption_animals(id) ON DELETE SET NULL
    )');
...
function ensureSettingDefaults(PDO $pdo): void
{
    $defaults = [
        'site_title' => 'Feroxz',
        'site_tagline' => 'Reptilienverwaltung & Genetik',
        'footer_text' => 'Inhalte werden sicher auf dem Server verwaltet. Besuche uns für aktuelle Projekte und Pflegewissen.',
        'contact_email' => 'info@example.com',
        'home_hero_title' => 'Reptilien nachhaltig züchten & vermitteln',
        'home_hero_subtitle' => 'Pflegeleitfäden, Genetik-Tools und ein moderner Tierbestand – alles in einem CMS.',
        'adoption_intro' => 'Hier findest du Reptilien aus verantwortungsvoller Haltung, die ein neues Zuhause suchen. Jedes Tier wird mit vollständiger Genetik und Pflegehistorie vorgestellt.'
    ];

    $stmt = $pdo->prepare('SELECT key FROM settings WHERE key = ?');
    $insert = $pdo->prepare('INSERT INTO settings (key, value, updated_at) VALUES (?, ?, ?)');

    foreach ($defaults as $key => $value) {
        $stmt->execute([$key]);
        if (!$stmt->fetchColumn()) {
            $insert->execute([$key, $value, date(DATE_ATOM)]);
        }
    }
}
...
function render(string $view, array $params = [], string $layout = 'layout'): void
{
    ...
    if (!array_key_exists('settings', $params)) {
        $params['settings'] = loadSettings($pdo);
    }
...
function permissionLabels(): array
{
    return [
        ...
        'gallery' => 'Galerie',
        'animals' => 'Tiere',
        'adoptions' => 'Tierabgabe',
        'genetics' => 'Genetikdaten',
        'settings' => 'Seitentexte & Einstellungen',
...
function adminNavigation(?array $user): array
{
    $items = [
        ...
        ['label' => 'Tierabgabe', 'route' => 'admin/adoptions', 'permission' => 'adoptions'],
        ['label' => 'Genetik', 'route' => 'admin/genetics', 'permission' => 'genetics'],
        ['label' => 'Einstellungen', 'route' => 'admin/settings', 'permission' => 'settings'],
...
function handleAdoptions(PDO $pdo, string $method): void
{
    $statusOptions = adoptionStatusOptions();
    $inquiryStatusOptions = adoptionInquiryStatusOptions();
    ...
    if ($method === 'POST') {
        ...
        if ($action === 'delete' && $id) {
            deleteAdoptionAnimal($pdo, $id);
            setFlash('Tier wurde aus der Abgabeliste entfernt.', 'success');
            header('Location: ' . url('admin/adoptions'));
            exit;
        }
        ...
        if ($action === 'inquiry-status') {
            ...
            if ($inquiryId && isset($inquiryStatusOptions[$status])) {
                updateAdoptionInquiryStatus($pdo, $inquiryId, $status);
                setFlash('Anfrage aktualisiert.', 'success');
            }
            header('Location: ' . url('admin/adoptions'));
            exit;
        }
        ...
        if ($action === 'save') {
            ...
            if ($name === '' || $species === '') {
                setFlash('Bitte Name und Art des Tiers angeben.', 'error');
                header('Location: ' . url('admin/adoptions', ['id' => $id ?: null]));
                exit;
            }
            ...
            $existing = null;
            if ($id) {
                $existing = findAdoptionAnimal($pdo, $id);
                if (!$existing) {
                    setFlash('Tier nicht gefunden.', 'error');
                    header('Location: ' . url('admin/adoptions'));
                    exit;
                }
            ...
            if (!empty($_POST['remove_image']) && $imagePath) {
                $relative = ltrim(parse_url($imagePath, PHP_URL_PATH), '/');
            ...
            $adoptionId = saveAdoptionAnimal($pdo, [
                'id' => $id,
                'name' => $name,
                'species' => $species,
                'genetics' => $genetics,
            ...
            setFlash($id ? 'Tier aktualisiert.' : 'Tier zur Abgabe angelegt.', 'success');
            header('Location: ' . url('admin/adoptions', ['id' => $adoptionId]));
            exit;
        }
    ...
    $editId = isset($_GET['id']) ? (int)$_GET['id'] : null;
    $editAdoption = $editId ? findAdoptionAnimal($pdo, $editId) : null;

    renderAdmin('adoptions/index', [
        'adoptions' => fetchAdoptionAnimals($pdo),
        'editAdoption' => $editAdoption,
        'statusOptions' => $statusOptions,
        'inquiries' => fetchAdoptionInquiries($pdo),
...
function handleAdoptionInquiry(PDO $pdo, string $method): void
{
    if ($method !== 'POST') {
        header('Location: ' . url('adoption'));
        exit;
    }

    $settings = loadSettings($pdo);
    $adoptionId = isset($_POST['adoption_id']) ? (int)$_POST['adoption_id'] : null;
    $adoption = $adoptionId ? findAdoptionAnimal($pdo, $adoptionId) : null;
    $adoptionName = trim($_POST['adoption_name'] ?? '');
    if ($adoption && $adoptionName === '') {
        $adoptionName = $adoption['name'];
    }
    if ($adoptionName === '') {
        $adoptionName = 'Tierabgabe';
    }
    ...
    if ($name === '' || !filter_var($email, FILTER_VALIDATE_EMAIL)) {
        setFlash('Bitte Name und eine gültige E-Mail-Adresse angeben.', 'error');
        header('Location: ' . url('adoption'));
        exit;
    }
    ...
    saveAdoptionInquiry($pdo, [
        'adoption_id' => $adoption ? $adoption['id'] : null,
        'adoption_name' => $adoptionName,
    ...
    $targetEmail = settingValue($settings, 'contact_email', '');
    if (filter_var($targetEmail, FILTER_VALIDATE_EMAIL)) {
        $subject = 'Neue Tierabgabe-Anfrage: ' . $adoptionName;
        $bodyLines = [
            'Tier: ' . $adoptionName,
            'Interessent: ' . $name,
            'E-Mail: ' . $email,
        ...
        if (function_exists('mail')) {
            ...
            @mail($targetEmail, $subject, implode("\n", $bodyLines), $headers);
        }
    }

    setFlash('Vielen Dank für deine Anfrage! Wir melden uns so schnell wie möglich.', 'success');
    header('Location: ' . url('adoption'));
    exit;
}
...
function handleSettings(PDO $pdo, string $method): void
{
    ...
    if ($method === 'POST') {
        ...
        $adoptionIntro = trim($_POST['adoption_intro'] ?? '');

        if ($title === '') {
            $title = 'Feroxz';
        }
        if ($tagline === '') {
            $tagline = 'Reptilienverwaltung & Genetik';
        }
        ...
        updateSettings($pdo, [
            ...
            'adoption_intro' => $adoptionIntro
        ]);

        setFlash('Einstellungen gespeichert.', 'success');
...
function adoptionStatusOptions(): array
{
    return [
        'available' => 'verfügbar',
        'reserved' => 'reserviert',
...
function adoptionInquiryStatusOptions(): array
{
    return [
        'new' => 'Neu',
...
function fetchPublicAdoptions(PDO $pdo): array
{
    $stmt = $pdo->prepare('SELECT * FROM adoption_animals WHERE status != ? ORDER BY sort_order ASC, datetime(updated_at) DESC');
    $stmt->execute(['archived']);
    $adoptions = $stmt->fetchAll();
    foreach ($adoptions as &$animal) {
        $animal['price'] = (string)($animal['price'] ?? '');
    }
    return $adoptions;
}

function fetchAdoptionAnimals(PDO $pdo): array
{
    $stmt = $pdo->query('SELECT * FROM adoption_animals ORDER BY sort_order ASC, datetime(updated_at) DESC');
    $animals = $stmt->fetchAll();
    foreach ($animals as &$animal) {
        $animal['price'] = (string)($animal['price'] ?? '');
    }
...
function findAdoptionAnimal(PDO $pdo, int $id): ?array
{
    $stmt = $pdo->prepare('SELECT * FROM adoption_animals WHERE id = ?');
    $stmt->execute([$id]);
    $animal = $stmt->fetch();
    if ($animal) {
        $animal['price'] = (string)($animal['price'] ?? '');
...
function saveAdoptionAnimal(PDO $pdo, array $data): int
{
    $now = date(DATE_ATOM);
    $sortOrder = isset($data['sort_order']) ? (int)$data['sort_order'] : 0;
    if (!empty($data['id'])) {
        $stmt = $pdo->prepare('UPDATE adoption_animals SET name = ?, species = ?, genetics = ?, description = ?, birth_date = ?, price = ?, status = ?, image_path = ?, sort_order = ?, updated_at = ? WHERE id = ?');
        $stmt->execute([
            $data['name'],
            $data['species'],
            $data['genetics'],
    ...
    if ($sortOrder === 0) {
        $sortOrder = (int)$pdo->query('SELECT COALESCE(MAX(sort_order), 0) + 1 FROM adoption_animals')->fetchColumn();
    }

    $stmt = $pdo->prepare('INSERT INTO adoption_animals (name, species, genetics, description, birth_date, price, status, image_path, sort_order, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
    $stmt->execute([
        $data['name'],
        $data['species'],
        $data['genetics'],
...
function deleteAdoptionAnimal(PDO $pdo, int $id): void
{
    $stmt = $pdo->prepare('SELECT image_path FROM adoption_animals WHERE id = ?');
    $stmt->execute([$id]);
    $image = $stmt->fetchColumn();
    if ($image) {
        $relative = ltrim(parse_url($image, PHP_URL_PATH), '/');
        $path = __DIR__ . '/' . $relative;
        if (is_file($path)) {
            @unlink($path);
        }
    }
    $pdo->prepare('DELETE FROM adoption_animals WHERE id = ?')->execute([$id]);
}

function fetchAdoptionInquiries(PDO $pdo): array
{
    $stmt = $pdo->query('SELECT * FROM adoption_inquiries ORDER BY datetime(created_at) DESC');
    return $stmt->fetchAll();
}

function saveAdoptionInquiry(PDO $pdo, array $data): void
{
    $stmt = $pdo->prepare('INSERT INTO adoption_inquiries (adoption_id, adoption_name, interested_name, interested_email, message, created_at, status) VALUES (?, ?, ?, ?, ?, ?, ?)');
    $stmt->execute([
        $data['adoption_id'],
        $data['adoption_name'],
        $data['interested_name'],
        $data['interested_email'],
        $data['message'],
        date(DATE_ATOM),
        $data['status'] ?? 'new'
    ]);
...
function updateAdoptionInquiryStatus(PDO $pdo, int $id, string $status): void
{
    $stmt = $pdo->prepare('UPDATE adoption_inquiries SET status = ? WHERE id = ?');
